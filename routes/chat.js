...
exports.start = function(io) {
  ...
  io.sockets.on('connection', function(socket) {
    ...
    socket.on('sendchat', function(data) {
      ...
      io.sockets. in (socket.room).emit('updatechat', usernames[socket.id],
        data, datetimeNow());
      insertChatContent(socket.room, socket.userid, usernames[socket.id].login,
        usernames[socket.id].avatar, data);
    });
    ...
    socket.on('sendalarm', function(userid, data) {
      ...
      if (receiverSocketId != false) {
        io.sockets.socket(receiverSocketId).emit('updatealarm', usernames[
          socket.id], data);
      }
    });
