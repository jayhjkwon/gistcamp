  ...
  function(accessToken, refreshToken, profile, done) {
    process.nextTick(function() {
      ...
      async.parallel([
          ...
          function(cb) {
            user.getAllFollowings(null, null, accessToken, function(
              followings) {
          ...
          function(cb) {
            gist.getAllStarredGists(null, null, accessToken, function(
              allStarredGists) {
          ...
          function(cb) {
            User.findOne({
              ...
              if (!doc) {
                userToSave.gistcamp_joindate = moment.utc().format(
...
app.get('/auth/github',
  passport.authenticate('github', {
    scope: ['user', 'user:email', 'user:follow', 'repo', 'notifications',
      'gist'
    ]
  }),
  function(req, res) {}
...
/* restful services */
app.get('/api/server/options', ensureAuthenticated, function(req, res) {
  res.send({
    env: config.env
  });
});
...
app.post('/api/friends/watch/sort/:login_id/:new_index', ensureAuthenticated,
  user.sortWatch);
