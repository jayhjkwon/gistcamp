  ...
  $ = require('jquery'),
    ...
    CommentItemView = Marionette.ItemView.extend({
      ...
      initialize: function(options) {
        ...
        _.bindAll(this, 'onRender', 'onCommentClick',
          'onCommentEditKeypress', 'onCommentHover', 'deleteComment');
      ...
      deleteComment: function() {
        ...
        if (!self.deleting) {
          ...
          var comment = new CommentItem({
            gistId: self.gistItem.id,
            id: self.model.get('id')
          });
          comment.destroy()
            .done(function() {
              postalWrapper.publish(constants.COMMENT_DELETE, self.model.get(
                'id'));
            })
            .always(function() {
              self.deleting = false;
