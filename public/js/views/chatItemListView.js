define(function(require) {
  ...
  $ = require('jquery'),
    ...
    ChatItemListView = Marionette.CollectionView.extend({
      ...
      initialize: function() {
        _.bindAll(this, 'getChatList', 'addChatList', 'onClose',
          'removeChatList', 'onItemSelected', 'getChatHistory');

        this.spinner = new Spinner();
        this.subscriptionUpdateRoom = postalWrapper.subscribe(constants.CHAT_UPDATE_ROOM,
          this.getChatList);
        this.subscriptionCreateRoom = postalWrapper.subscribe(constants.CHAT_CREATE_ROOM,
          this.addChatList);
        this.subscriptionDeleteRoom = postalWrapper.subscribe(constants.CHAT_DELETE_ROOM,
          this.removeChatList);
        ...
        for (var i = 0; i < cotents.length; i++) {
          if (cotents[i].user_login === 'SERVER') {
            ...
            var server = '<div class="server">' + '<span class="message">' +
              cotents[i].content + '</span>' + '</div>';

            var right = '<div class="right">' + '<span class="time">' +
              moment(cotents[i].created_at).format('MM/DD/YYYY h:mm:ss A') +
              '</span>' + '</div>';
            ...
            var left = '<div class="left">' + '<img class="gravatar" src="' +
              cotents[i].avatar_url + '" </img>' + '<div class="name">' +
              cotents[i].user_login + '</div>' + '</div>';

            var middle = '<div class="middle">' + cotents[i].content +
              '</div>';
      ...
      loading: function(showSpinner) {
        if (showSpinner) {
          $('#chat-list').append(
            '<div style="height:100px;" class="loading"></div>');
