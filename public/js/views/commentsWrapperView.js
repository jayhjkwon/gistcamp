    ...
    CommentsWrapperView = Marionette.CompositeView.extend({
      ...
      initialize: function(options) {
        _.bindAll(this, 'onDomRefresh', 'onItemSelected',
          'onCommentInputKeypress', 'onCommentDeleted');
      ...
      itemViewOptions: function() {
        return {
          gistItem: this.selectedGistItem
        }; // gistItem will be passed to the itemView
      },
      ...
      onCommentInputKeypress: function(e) {
        ...
        if (keyCode === 13 && !self.saving) {
          ...
          var comment = new CommentItem({
            gistId: this.selectedGistItem.id,
            commentText: text
          });
          comment.save()
            .done(function(data) {
              ...
              if (self.selectedGistItem.user.id !== global.user.id) {
                var msg = global.user.name +
                  ' just commented on your gist(' + self.selectedGistItem
                  .description + ') ';
                msg = msg + '</br>';
                msg = text.length > 50 ? msg + '"' + text.substring(0, 50) +
                  '.."' : msg + '"' + text + '"';
                global.socket.emit('sendalarm', self.selectedGistItem.user
                  .id, msg);
              }
            })
            .always(function() {
      ...
      onItemSelected: function(gistItem) {
        ...
        if (gistItem.comments == 0) {
          ...
          self.loading(true);
          self.collection = new CommentItemList({
            gistId: gistItem.id
          });
