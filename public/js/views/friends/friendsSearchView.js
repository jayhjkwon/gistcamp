define(function(require) {
  ...
  $ = require('jquery'),
    ...
    friendsSearchTemplate = require(
      'hbs!templates/friends/friendsSearchTemplate'),
    ...
    FriendsSearchView = Marionette.CompositeView.extend({
      ...
      initialize: function(options) {
        ...
        _.bindAll(this, 'addCardView', 'removeCardView', 'onRender',
          'onDomRefresh', 'getFriends', 'getFollowing', 'getFollowers',
          'onScroll', 'loadMore', 'loading');
      ...
      onRender: function() {
        $('.friends-search-container').niceScroll({
          cursorcolor: '#eee'
        });
      ...
      onScroll: function() {
        ...
        console.log(w.scrollTop() + ', ' + w.height() + ', ' + (parseInt(w.scrollTop()) +
          parseInt(w.height())) + ', ' + $('.friends-search-view').height());
        if ((parseInt(w.scrollTop()) + parseInt(w.height())) >= this.$el.find(
          '.friends-search-view').height()) {
          this.loadMore();
        }
      },
