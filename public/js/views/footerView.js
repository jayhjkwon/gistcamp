define(function(require) {
  ...
  $ = require('jquery'),
    ...
    FooterView = Marionette.ItemView.extend({
      ...
      initialize: function() {
        _.bindAll(this, 'toggleCommentRegion', 'popupWindow', 'deleteTag',
          'sharePocket', 'shareEverNote', 'onTagItemHover', 'shareWo',
          'shareGg', 'shareFB', 'shareTW', 'shareFB',
          'initializePopOverTag', 'onItemSelected', 'star', 'createTag',
          'loading', 'onBtnCommentClick', 'onRoomCreated', 'tagOnGist',
          'onCommentDeleted', 'onCommentAdded', 'onTagCollectionChange',
          'initializePopOverShare');
        ...
        this.subscription = postalWrapper.subscribe(constants.GIST_ITEM_SELECTED,
          this.onItemSelected);
        ...
        this.subscriptionAddComment = postalWrapper.subscribe(constants.COMMENT_ADD,
          this.onCommentAdded);
        this.subscriptionStar = postalWrapper.subscribe(constants.STAR,
          this.star);
        this.subscriptionCommentRegion = postalWrapper.subscribe(constants.HIDE_COMMENT_REGION,
      ...
      initializePopOverTag: function() {
        ...
        this.ui.btnTag.popover({
          ...
          placement: 'top',
          title: function() {
            return '<div><i class="icon-tag"></i> Tag this gist</div>';
          },
          content: function() {
            return $('.tag-area').html();
          }
        });
      ...
      initializePopOverShare: function() {
        this.ui.btnShare.popover({
          ...
          placement: 'top',
          title: function() {
            return '<div><i class="icon-share"></i> Share this gist</div>';
          },
          content: function() {
            return $('.share-area').html();
          }
        });
      },
      ...
      onTagCollectionChange: function(event_name) {
        ...
        if (self.model) {
          ...
          if ($('.tag-popup').length) {
            self.$el.find('.tag-popup').replaceWith(tagListTemplate({
              tags: self.tags.toJSON()
            }));
          } else {
            $('.tag-area').html(tagListTemplate({
              tags: self.tags.toJSON()
            }));
          }
      ...
      star: function(e) {
        ...
        if (self.model.get('is_starred')) {
          service.deleteStar(this.model.get('id')).done(function(data) {
            ...
            postalWrapper.publish(constants.GIST_STAR_CHANGED, self.model
              .toJSON());
          }).always(function() {
      ...
      showStarActionMessage: function(isStarred) {
        if (isStarred) {
          $('.starred-success').text('Starred Successfully').removeClass(
          ...
          $('.starred-success').text('Unstarred Successfully').removeClass(
            'starred-success-hide starred-success-show').addClass(
            'starred-success-show');
          setTimeout(function() {
            $('.starred-success').removeClass(
      ...
      onBtnCommentClick: function(e) {
        e.preventDefault();
        this.toggleCommentRegion();
      },

      toggleCommentRegion: function() {
      ...
      onItemSelected: function(gistItem) {
        ...
        if (gistItem && gistItem.comments > 0) {
          $('.comments-badge').addClass('show-comments-badge').text(
            gistItem.comments);
          this.showCommentRegion();
        } else {
      ...
      shareTW: function(e) {
        e.preventDefault();
        var gistUrl = this.model.get('html_url');
        var url = 'https://twitter.com/intent/tweet?via=gistcamp&url=' +
          encodeURIComponent(gistUrl);
        var title = 'GistCamp';
      ...
      shareGg: function(e) {
        e.preventDefault();
        var gistUrl = this.model.get('html_url');
        var url = 'https://plus.google.com/share?url=' + encodeURIComponent(
          gistUrl);
        var title = 'GistCamp';
      ...
      sharePocket: function(e) {
        ...
        var url = 'https://getpocket.com/save?url=' + encodeURIComponent(
          gistUrl) + "&title=" + encodeURIComponent(description);
        var title = 'GistCamp';
        ...
        service.isEvernoteAuthenticated().done(function(result) {
          if (result.authenticated) {
            ...
            service.saveNote(self.model.get('id')).done(function(result) {
              ...
              setTimeout(function() {
                $(e.target).siblings('.saved').removeClass(
                  'saved-show');
              }, 3000);
            }).always(function() {
              self.loading(false);
      ...
      popupWindow: function(url, title, w, h) {
        ...
        newWindow = window.open(url, title, 'scrollbars=yes, width=' + w +
      ...
      onClose: function() {
        ...
        this.subscriptionCommentRegion.unsubscribe();
      }
    });
